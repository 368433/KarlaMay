...
func auth0TokenRequest(tokenEndpoint: String, grantType: GrantType, clientID: String, clientSecret: String, authorizationCode: String? = nil, redirectUri: String? = nil, completion: @escaping (AuthOTokenResponsePayload) -> ()) {
    guard let endpoint = URL(string: tokenEndpoint) else {return}
    guard let grantType = "grant_type=\(grantType.rawValue)".data(using: String.Encoding.utf8) else {return}
    ...
    request.httpBody = grantType + clientID + secret
